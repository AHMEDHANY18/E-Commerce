...
export const createOrder = asyncHandler(async (req, res, next) => {
    ...
    if (productId) {
        ...
        if (!cart.products.length) {
            return next(new Error("cart is empty please select a product to order", 404))
        }
    ...
    await sendMail(req.user.email, "Order Details", `<p>Order Details</p>`, [
        {
            path: "invoice.pdf",
...
export const cancelOrder = asyncHandler(async (req, res, next) => {
    ...
    if ((order.paymentMethod === "cash" && order.status != "placed") || (order.paymentMethod === "card" && order.status != "waitPayment")) {
        return next(new Error("you can not cancel this order", 400))
    }
...
export const getOrders = asyncHandler(async (req, res, next) => {
    // Initialize the apiFeature with the Order model and request query
    const apiFeatureInstance = new ApiFeatures(orderModel.find(), req.query)
        .pagination()
        .filter()
        .sort()
        .select()
    ...
    // Execute the mongoose query generated by apiFeature
    const orders = await apiFeatureInstance.mongooseQuery;

    // Send the response with the fetched orders
    res.status(200).json({ msg: "done", page: apiFeatureInstance.page, orders });
});